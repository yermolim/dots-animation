...
class Dot implements IAnimationMovable {
...
class AnimationCanvas implements IAnimationCanvas {
    ...
    constructor(parent: HTMLElement, canvasId: string,
        ...
        this._animationControl = AnimationCanvas.animationControlFactory(constructor, this._canvas, options);
    }
...
class DotsAnimation {    
    static options : IAnimationOptions = {       
        expectedFps: 60, 
        minR: 1,
        maxR: 5,
        minSpeedX: -0.5,
    ...
    static async main(containerSelector: string, canvasId: string, optionsJsonPath: string): Promise<IAnimationCanvas> {
        let options: IAnimationOptions = DotsAnimation.options;
        ...
        return new AnimationCanvas(container as HTMLElement, canvasId, options, DotControl);
    }
}
